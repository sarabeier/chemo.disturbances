...
# Load predicted traits for data re-transformation
```{r}
pic.traits <- read.table("../data.all/picrust.traits/trait.files.new/pic.traits_trans.tab", header=T, sep ='\t')
tibble(pic.traits)
```
...
pic.16s.custom <- read.table("../data.all/picrust.traits/predicted.new/pic.chemo10.16S_predicted_custom_tree.txt", header=T) #SARA: please simplify path, everything in a folder ../PICRUSt2 should be fine
tibble(pic.16s.custom)
#load generation time predicted from PICRUSt2 default tree and database
pic.d.gRodon.default <- read.table("../data.all/picrust.traits/predicted.new/p.d.gRodon.scaled.txt", header=T)  
tibble(pic.d.gRodon.default)

#Resistance-related genes
#load %TF predicted from PICRUSt2 default tree and database
pic.TFr.default <- read.table("../data.all/picrust.traits/predicted.new/p.TF_perc.scaled.txt", header=T)
tibble(pic.TFr.default)
#load genome size predicted from PICRUSt2 default tree and database
pic.gs.default <- read.table("../data.all/picrust.traits/predicted.new/p.genome.size.scaled.txt", header=T)
...
```{r}
#SARA: we do not need the 16s default data(they are not needed anywhere else), we can simply use each of the predicted traits from the default reference set
...
tibble(res.ttest.df)
```
## Figure XX (#SARA: display final Figure nr)
```{r, fig.dim=c(5.5,6)}
# Edit the title figures with Pvalues #SARA: ? Could you add the trait inside the panel behind the title (this will save space)
...
bxp=tag_facet(bxp,open = "", close = "",
              tag_pool=c(" A) RNN", " B) RNN "," C) d"," D) d"," E) TF"," F) TF"," G) Genome Size", " H) Genome Size"," I) Shannon"," J) Shannon"),x=0, fontface = 1,size=3,hjust=0)
